...
Matrix* matmul_SIMD_v2(const Matrix* A, const Matrix* B){
    Matrix* BT = transpMatrix(B);
    if (A == NULL || BT == NULL)
    {
        ERROR_INPUT_POINTER;
        printf("ERROR: This error happened in 'matmul_plain_row' \n");
    ...
    for(i=0; i < A->row; i++)
    {
        for(j=0; j < B->row; j++)
        {
            ...
            {
                a = _mm_loadu_ps(A->data + (i*A->column + k));
                b = _mm_loadu_ps(BT->data + (j*BT->column + k));
                c = _mm_add_ps(c, _mm_mul_ps(a, b));
            }
            _mm_store_ps(sum, c);
            C_data[i*A->row + j] = (sum[0] + sum[1] +sum[2] +sum[3]);
        }
    }
